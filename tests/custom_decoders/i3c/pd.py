# Vendored from https://github.com/xyphro/Sigrok-I3C-decoder (branch: main,
# path: i3c/pd.py), a third-party sigrok protocol decoder for MIPI I3C.
# Not part of mainline libsigrokdecode. Included here, unmodified below this
# comment block, strictly as an external validation oracle for
# tests/test_sigrok_roundtrip.py (invoked via `sigrok-cli` with
# SIGROKDECODE_DIR pointed at tests/custom_decoders/, exactly like every
# mainline sigrok decoder this project already shells out to) — never
# imported directly by protowavegen itself.
#
# Copyright (C) 2025 Kai Gossner <xyphro@gmail.com>
# Licensed under the GNU General Public License v3.0 (or later) — see the
# LICENSE file in this directory (tests/custom_decoders/LICENSE), copied
# verbatim from the upstream repository.
#
# One local, behavior-preserving edit below: the upstream
# `from common.srdhelper import bitpack_msb` import is removed. It is dead
# code (grep confirms `bitpack_msb` is never referenced anywhere else in
# this file) that fails to import against this environment's installed
# libsigrokdecode `common.srdhelper` module (which doesn't export that
# name in this version) — confirmed via `sigrok-cli`'s own ImportError
# before this edit. Removing an unused import changes no decoding
# behavior; everything else below is unmodified upstream source.
