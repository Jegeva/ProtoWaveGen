...
# Neither MIPI I3C nor IrDA has a mainline sigrok decoder we can rely on as
# an independent oracle as-is — I3C's round-trip test below uses a vendored
# third-party decoder instead (see tests/custom_decoders/i3c/'s own header
# comment for provenance/license), and IrDA's uses a decoder this project
# wrote itself (tests/custom_decoders/irda/pd.py). Both are loaded by
# pointing sigrok-cli's SIGROKDECODE_DIR at their shared parent directory,
# in *addition* to (not replacing) the system decoder path, exactly like
# every other decoder in this file — confirmed empirically (`sigrok-cli -l
# 4` lists both the system decoder path and this one).
